...
ancom_out="${results_folder}/genus_ancom_results"
...
echo "✅ 开始按 Genus 分组并运行 ANCOM..." | tee "$log"

# Step 1: collapse to Genus level
echo "➡️ Collapsing to Genus level..." | tee -a "$log"
qiime taxa collapse \
  --i-table "$input_table" \
  --i-taxonomy "$taxonomy_file" \
  --p-level 6 \
  --o-collapsed-table "${ancom_out}/genus_table.qza"

# Step 2: Add pseudocount
echo "➡️ Adding pseudocount..." | tee -a "$log"
qiime composition add-pseudocount \
  --i-table "${ancom_out}/genus_table.qza" \
  --o-composition-table "${ancom_out}/genus_composition.qza"
...
for col in "${columns[@]}"; do
  ...
  qiime composition ancom \
    --i-table "${ancom_out}/genus_composition.qza" \
    --m-metadata-file "$metadata_file" \
    --m-metadata-column "$col" \
    --o-visualization "${ancom_out}/genus_ancom_${col}.qzv"

  if [ $? -eq 0 ]; then
    echo "     ✅ '$col' ANCOM complete" | tee -a "$log"
  else
    echo "     ❌ '$col' failed, please check metadata" | tee -a "$log"
  fi
done

echo "������ ANCOM completed, outputs in directory：$ancom_out" | tee -a "$log"
