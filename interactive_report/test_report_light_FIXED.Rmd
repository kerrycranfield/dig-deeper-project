...
rare_df <- read_csv("data/16S/s06_Results/Alpha_rarefaction/data/observed_features.csv")
...
rare_avg <- rare_long %>%
  group_by(Establishment.y, depth) %>%
  summarise(mean_obs = mean(observed, na.rm = TRUE), .groups = "drop")

plot_rare <- ggplot(rare_avg, aes(x = depth, y = mean_obs, color = Establishment.y)) +
  geom_line(size = 0.5) + 
  geom_point(size = 1, shape =0)+
  theme_minimal() +
  labs(
    title = "Rarefaction Curves by Establishment (Mean per Depth)",
    x = "Sequencing Depth",
    y = "Mean Observed Features"
  )
  ...
  if (file.exists(path_ancom) && file.exists(path_data)) {
    ...
    p <- ggplot(df, aes(x = clr, y = W, color = `Reject.null.hypothesis`, text = X)) +
      ...
      labs(x = "Centered Logarithmic (clr)",
       y = "W statistic",title = paste("ANCOM Volcano-like plot (16S) for", v)) +
      theme(
    ...
    cat(sprintf('<p style="text-align: center;"><a href="%s" target="_blank">View interactive volcano-like plot in QIIME2 (16S - %s)</a></p>', path_link, v))
    cat(as.character(htmltools::div(style = "text-align: center;", ggplotly(p))))
    cat(sprintf("</br><p style='text-align: center; font-weight: bold;'>Figure xx. Differences in Taxa Abundance Associated with '%s' (ANCOM Results) (16S)</p></br>", v))
...
function switchPlotANCOM_ITS() {
  ...
  all.forEach(function(v) {
    var el = document.getElementById("plot_ANCOM_ITS" + v);
    el.style.display = (v === val) ? "block" : "none";
  });
}
</script>
...
for (v in vars) {
  ...
  if (file.exists(path_ancom) && file.exists(path_data)) {
    ...
    p <- ggplot(df, aes(x = clr, y = W, color = `Reject.null.hypothesis`, text = X)) +
      ...
      theme_minimal() +
      labs(x = "Centered Logarithmic (clr)",
       y = "W statistic",title = paste("ANCOM Volcano-like plot (ITS) for", v)) +
      theme(
        axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
        legend.position = "bottom"  
    ...
    cat(sprintf('<div id="plot_ANCOM_ITS%s" style="display:%s;">', v, div_style))
    ...
    cat(as.character(htmltools::div(style = "text-align: center;", ggplotly(p))))
    cat(sprintf("</br><p style='text-align: center; font-weight: bold;'>Figure xx. Differences in Taxa Abundance Associated with '%s' (ANCOM Results) (ITS)</p></br>", v))
...
for (v in vars) {
    ...
    ancom_path <- paste0("data/16S/Taxonomy/ancom_pathway/pathway_ancom_", v, "/data/ancom.tsv" )
    data_path <- paste0("data/16S/Taxonomy/ancom_pathway/pathway_ancom_", v, "/data/data.tsv" )
    ...
    cat(as.character(htmltools::div(style = "text-align: center;", ggplotly(p))))
    cat(sprintf("</br><p style='text-align: center; font-weight: bold;'>Figure xx. Differences in Functional Pathways Associated with '%s' (ANCOM Results) (16S)</p></br>", v))
...
# Ordonner les sites par Establishment
...
# Préparer les séparateurs et les labels
group_breaks <- site_groups %>%
  group_by(Establishment) %>%
  summarise(n = n()) %>%
  mutate(position = cumsum(n) + 0.5) %>%
...
HTML("<p style='text-align: center; font-weight: bold;'>Figure xx. Fungal Guild Abundance by Site</p>")
...
cat('
<iframe src="plots/corr_plots_16S_spearman.html"  width="100%" height="400" frameborder="0" scrolling="no"></iframe>
')
